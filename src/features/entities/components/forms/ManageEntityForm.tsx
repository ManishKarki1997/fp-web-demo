...
import { useCreateEntityMutation, useEditEntityMutation } from '../../api/entityApi'
...
const ManageEntityForm = ({ isOpen, onClose, selectedEntity }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof CreateEntitySchema>) => {
    ...
    if (selectedEntity) {
      ...
      const loadingId = toast.loading(t("Editing Entity"))
      try {
        await editEntityMutation({ ...values, id: selectedEntity.id }).unwrap()
        toast.dismiss(loadingId)
        toast.success(t("Entity Edited Success"))
        form.reset()
        onClose()
      } catch (error) {
      ...
      const loadingId = toast.loading(t("Adding Entity"))
      try {
        await createEntityMutation(values).unwrap()
        toast.dismiss(loadingId)
        toast.success(t("Entity Added Success"))
        form.reset()
        onClose()
      } catch (error) {
        const errorMessage = error?.data.message || t("Something went wrong")
